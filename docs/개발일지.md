...
## 2025-04-13 11:30 - 데이터베이스 스키마 문제 해결 및 테이블 생성

### 문제 상황
문제 입력 기능을 테스트하는 중 API 요청 시 다음과 같은 오류가 발생했습니다:
```
relation "questions" does not exist
```

API 응답에 500 Internal Server Error가 반환되었으며, 데이터베이스에 필요한 테이블이 생성되지 않은 것으로 확인되었습니다.

### 원인 분석
1. **스키마 정의와 실제 DB 불일치**:
   - Drizzle ORM에서 스키마가 정의되었지만, 실제 데이터베이스에 테이블이 생성되지 않았습니다.
   - 마이그레이션 스크립트가 실행되지 않았거나 오류가 발생했습니다.

2. **마이그레이션 실행 시 발생한 문제**:
   - 스키마 파일(`db/schema/index.ts`, `db/schema/questions.ts`, `db/schema/images.ts`) 간 참조 관계에 순환 종속성이 있었습니다.
   - `imageHistory` 테이블이 정의되기 전에 `users` 테이블을 참조하는 문제가 있었습니다.
   - `images` 테이블이 참조되지만 해당 테이블이 아직 생성되지 않았습니다.

3. **esbuild 관련 오류**:
   - Drizzle 마이그레이션 생성 시 ES5 대상 환경에서 const 변환 지원 오류가 발생했습니다.
   - TypeScript 설정 문제로 마이그레이션 생성이 중단되었습니다.

### 해결 과정

#### 1. TypeScript 설정 수정
tsconfig.json의 target 설정을 ES5에서 ES2020으로 변경했습니다:
```json
{
  "compilerOptions": {
    "target": "ES2020", // 기존: "es5"
    // ... 다른 설정들 ...
  }
}
```
...
#### 2. 스키마 정의 순서 수정
`db/schema/index.ts` 파일에서 순환 참조를 해결하기 위해 테이블 정의 순서를 변경했습니다:
```typescript
// 먼저 users 테이블 정의
...
// 그 다음에 imageHistory 테이블 정의 (users 테이블 참조)
...
#### 3. 마이그레이션 생성 실패 해결
`images` 테이블 참조 문제로 인해 마이그레이션이 계속 실패했습니다. 최종적으로 직접 SQL 스크립트를 작성하여 필요한 테이블을 생성하기로 결정했습니다.

#### 4. 직접 SQL 스크립트 작성 및 실행
1. 이미지 테이블 생성 SQL:
```sql
-- 이미지 상태 ENUM 생성
DO $$ BEGIN
...
```bash
# 이미지 테이블 생성
cat setup-images.sql | docker-compose -f docker-compose.dev.yml exec -T db psql -U postgres -d quizapp

# 나머지 핵심 테이블 생성
cat setup-tables.sql | docker-compose -f docker-compose.dev.yml exec -T db psql -U postgres -d quizapp
```

### 결과
1. 성공적으로 모든 필요 테이블이 생성되었습니다:
   - `images`: 이미지 정보 저장
   - `users`: 사용자 정보 저장 
   - `questions`: 문제 정보 저장
   - `tags`: 태그 정보 저장
   - `question_tags`: 문제-태그 관계 저장

2. API 요청 시 "relation questions does not exist" 오류가 해결되었습니다.

### 배운 점
1. **ORM과 직접 SQL의 장단점**:
   - ORM은 타입 안전성과 코드 수준의 스키마 관리가 가능하지만 복잡한 종속성이 있는 경우 문제가 발생할 수 있음
   - 직접 SQL 스크립트는 더 명확한 제어가 가능하지만 타입 안전성이 부족함

2. **스키마 설계 시 고려사항**:
   - 테이블 간 참조 관계와 정의 순서가 중요함
   - 순환 참조를 피하거나 적절히 처리해야 함

3. **개발 환경 설정의 중요성**:
   - TypeScript와 빌드 도구 설정이 개발 과정에 큰 영향을 미침
   - ES 버전 간 호환성 문제가 예상치 못한 오류를 발생시킬 수 있음

### 향후 개선 사항
1. Drizzle 마이그레이션을 안정적으로 구성하여 스키마 변경 관리 자동화
2. 스키마 변경 이력 관리를 위한 버전 관리 시스템 구축
3. 개발/테스트/프로덕션 환경 간 데이터베이스 동기화 전략 수립
