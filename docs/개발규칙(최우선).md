## Docker Base 개발
-   **1. Docker를 사용하여 전체 개발 환경을 구성하세요.**
-   **2. Dockerfile을 사용하여 이미지를 빌드하세요.**
-   **3. 모든 코드는 모듈화 해서 효율적으로 사용합니다.**
-   **4. 한 파일당 최대 500라인을 넘지 않도록 코드를 설계합니다.**
-   **5. 500라인이 넘어가면 가급적 파일을 분할하여 작성함니다.**
-   **6. 개발시 개발컨테이너를 운영합니다.**
-   **7. 컨테이너 명은 미리 정해, 정한대로 생성합니다. 현재 컨테이너명은 quizapp-dev, quizapp-db-dev 입니다.**
-   **8. 프로젝트 루트는 /mnt/user/appdata/quizapp 으로 지정해서 터미널 명령어를 실행합니다.**
-   **9. 도커환경을 고려해서 프로젝트루트 이하는 상대경로로 사용합니다.**
-   **10. 네트워크는 ngnet으로 통신합니다.**
-   **11. 코드개선후에는 /docs/항상 파일별코드현황.md를 업데이트합니다.**
-   **12. 코드개선후에는 개선목적 및 수정사항을 반영한 개발일지를 docs/개발일지/ 에작성합니다. 개발일지 작성시 500라인을 넘어가게 되면, 다음 개발일지는 파일을 새로 생성하여 입력합니다.**
