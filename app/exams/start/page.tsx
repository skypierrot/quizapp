...
export default function ExamStartPage() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // 숫자키(1~N)로 선택지 선택
      if (e.key >= '1' && e.key <= String(shuffledOptions.length)) {
        const idx = Number(e.key) - 1;
        if (shuffledOptions[idx]) {
          handleAnswerSelect(questions[currentQuestionIndex]?.id, idx);
        }
      }
      // 좌우 화살표로 문제 이동
      ...
      {currentQuestion && (
        <Card key={currentQuestion.id}>
          ...
          <CardContent>
            ...
            {/* 선택지 (isShuffled에 따라 분기) */}
            <div className="space-y-2">
              {(isShuffled ? shuffledOptions : currentQuestion.options).map((option, index) => {
                      ...
                      <div className="ml-6 mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {option.images.map((image, imgIndex) => (
                          <CommonImage
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              imageZoom.showZoom(getImageUrl(image.url));
                            }}
                          />
                        ))}
                      </div>
