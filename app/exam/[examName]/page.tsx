...
// Rename component to reflect its purpose for exams
export default function ExamSessionSelectionPage() {
  ...
  // Define breadcrumb items dynamically for exam session selection
  const breadcrumbItems = decodedExamName ? [
    { label: '홈', href: '/' },
    { label: '모의고사 선택', href: '/exam' }, // Update breadcrumb path
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      {/* Display the decoded exam name as the page title and add a subtitle */}
      <h1 className="text-3xl font-bold">{decodedExamName}</h1>
      <p className="text-lg text-gray-600 mb-6">응시할 모의고사 회차를 선택하세요.</p>
      
      {/* Show message if no instances found */}
      {!loading && !error && examInstances.length === 0 ? (
        <p className="text-gray-500">해당 시험으로 등록된 회차가 없습니다.</p>
      ) : (
        // Render the grid of exam instance cards
        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 gap-4">
          {examInstances.map((instance) => {
            ...
            // Construct the URL for the study page with mode=exam parameter
            const studyUrl = `/study/${encodedExamNameParam}/${encodedYearParam}/${encodedSessionParam}?mode=exam`; 
