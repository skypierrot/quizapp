...
export function ResolvedQuestions({ 
  ...
  const handleChoiceChange = (id: string, index: number, value: string) => {
    ...
    if (newOptions[index]) {
      newOptions[index] = { 
        ...
        text: value, 
        number: index + 1,
        images: newOptions[index].images || []
      };
    }
  ...
  const handleImageUpload = async (id: string, file: File) => {
    ...
    // imageUrl 속성은 IQuestion에 없으므로 제거
                ...
                {questions.map((question, index) => (
                  ...
                  >
                    <span className="truncate">
                      {index + 1}. {question.content.substring(0, 30)}
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    {question.options.map((option, index) => (
                      <div key={`option-${question.id}-${index}`} className="flex items-center gap-2">
                  ...
                  <div className="space-y-4">
                    <Label>정답</Label>
                    <Select value={question.answer?.toString() || ''} onValueChange={(value) => handleQuestionChange(question.id, 'answer', parseInt(value))}>
                      <SelectTrigger>
                  ...
                  <div className="space-y-4">
                    <Label>이미지</Label>
                    <Input
                      value={question.images?.[0]?.url || ''}
                      onChange={(e) => handleQuestionChange(question.id, 'images', [{ url: e.target.value, hash: '' }])}
                      placeholder="이미지 URL"
                    />
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleQuestionChange(question.id, 'tags', e.target.value.split(',').map(tag => tag.trim()))}
